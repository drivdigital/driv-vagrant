...
class ScriptInstaller {
  ...
  public function __construct( $siteSetting ) {
    ...
    $this->debug       = getenv( 'VAGRANT_TEST' ) == 1;
  ...
  public function install( $provisionScript ) {
    ...
    foreach ( $classes as $class ) {
      ...
      if ( ! is_subclass_of( $class, '\DrivDigital\Vagrant\Provisioners\ScriptInstaller' ) ) {
        continue;
      }

      /**
       * @var \DrivDigital\Vagrant\Provisioners\ScriptInstaller $installer
       */
      $installer = new $class( $this->siteSetting );
